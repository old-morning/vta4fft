...
def test_gemm():
    def run_gemm_packed(env, remote, batch_size, channel, block):
        ...
        def verify(s):
            ...
            ## data.h
            with open ("data2.h", "w") as f:
                inp = data_packed.reshape(batch_size*channel)
                f.write("int8_t inp[] = {\n")
                for i in range(batch_size*channel-1):
                    f.write('{:d}, '.format(inp[i]))
                f.write('{:d}\n '.format(inp[batch_size*channel-1]))
                f.write('};\n')

                wgt = weight_packed.reshape(channel*channel)
                f.write("int8_t wgt[] = {\n")
                for i in range(channel*channel-1):
                    f.write('{:d},'.format(wgt[i]))
                f.write('{:d}\n'.format(wgt[channel*channel-1]))
                f.write('};\n')
                
                ref = res_ref.reshape(batch_size*channel)
                f.write("int8_t res[] = {\n")
                for i in range(batch_size*channel-1):
                    f.write('{:d},'.format(ref[i]))
                f.write('{:d}\n'.format(ref[batch_size*channel-1]))
                f.write('};\n')
    ...
    def _run(env, remote):
        print("========GEMM 256=========")
        run_gemm_packed(env, remote, 256, 16, 16)
