...
def test_gemm():
    def run_gemm_packed(env, remote, batch_size, channel, block):
        ...
        res_shf = te.compute(res_shape, lambda *i: res_gem(*i) >> 16, name="res_shf")
        ...
        def verify(s):
            ...
            data_orig = np.random.randint(-128, 127, size=(batch_size, channel)).astype(data.dtype)
            weight_orig = np.random.randint(-128, 127, size=(channel, channel)).astype(weight.dtype)
            ...
            res_ref = np.right_shift(res_ref, 16)
            res_ref = np.clip(res_ref, 0, (1 << (env.INP_WIDTH - 1)) - 1).astype(res.dtype)

            print("----- data.h-------")
            
            print("env.BLOCK_IN",env.BLOCK_IN)
            print("env.BLOCK_OUT",env.BLOCK_OUT)
            print("env.BATCH",env.BATCH)
            print("acc_dtype",env.acc_dtype)
            print("1 << (env.INP_WIDTH - 1))",1 << (env.INP_WIDTH - 1))
            print("res_dtype",res.dtype)
            print("env.TARGET",env.TARGET)
            print("env.inp_dtype",env.inp_dtype)
            ...
            #     inp = data_packed.reshape(128*128)
            #     f.write("int16_t inp[] = {\n")
            #     for i in range(128*128-1):
            #         f.write('{:d}, '.format(inp[i]))
            #     f.write('{:d}\n '.format(inp[128*128-1]))
            #     f.write('};\n')

            #     wgt = weight_packed.reshape(128*128)
            #     f.write("int16_t wgt[] = {\n")
            #     for i in range(128*128-1):
            #         f.write('{:d}, '.format(wgt[i]))
            #     f.write('{:d}\n'.format(wgt[128*128-1]))
            #     f.write('};\n')

            #     ref = res_ref.reshape(128*128)
            #     f.write("int16_t res[] = {\n")
            #     for i in range(128*128-1):
            #         f.write('{:d}, '.format(ref[i]))
            #     f.write('{:d}\n'.format(ref[128*128-1]))
            #     f.write('};\n')
    ...
    def _run(env, remote):
        print("========GEMM 8=========")
        run_gemm_packed(env, remote, 64, 64, 64)
